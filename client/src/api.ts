...
const registerToInterRep = async (idCommitment: BigInt) => {
    const result = await axios.post(`${INTERREP_API_BASE_URL}/register`, {'identity': idCommitment.toString()})
    return result.data;
};


const visitApp = async (identitySecret: bigint, witness: any, rlnIdentifier: bigint, interRepGroup: string, epoch: string, url: string) => {

    epoch = RLN.genExternalNullifier(epoch);
    const fullProof = await RLN.genProofFromBuiltTree(identitySecret, witness, epoch, url, rlnIdentifier, CIRCUIT_PATH, PROVER_KEY_PATH)
    ...
    const res = await axios.post(`${RATE_LIMITING_SERVER_BASE_URL}/users/access`, request);
